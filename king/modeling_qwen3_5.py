...
from configuration_qwen3_5 import QuasarConfig, QuasarTextConfig, QuasarVisionConfig
...
class QuasarVisionRotaryEmbedding(nn.Module):
...
class QuasarTextRotaryEmbedding(nn.Module):
    inv_freq: torch.Tensor  # fix linting for `register_buffer`

    def __init__(self, config: QuasarTextConfig, device=None):
        super().__init__()
    ...
    def compute_default_rope_parameters(
        config: QuasarTextConfig | None = None,
    ...
    def forward(self, x, position_ids):
        # In contrast to other models, Quasar has different position ids for the grids
...
class QuasarRMSNormGated(nn.Module):
...
class QuasarGatedDeltaNet(nn.Module):
    ...
    def forward(
        ...
        cache_params: QuasarDynamicCache | None = None,
...
class QuasarMLP(nn.Module):
    def __init__(self, config: QuasarConfig, intermediate_size: int):
        super().__init__()
        self.config = config
        self.hidden_size = config.hidden_size
...
class QuasarRMSNorm(nn.Module):
    ...
    def forward(self, x):
        ...
        # Llama does x.to(float16) * w whilst Quasar is (x * w).to(float16)
...
class QuasarGatedLinearAttention(nn.Module):
    def __init__(self, config: QuasarConfig, layer_idx: int):
        super().__init__()
...
class QuasarDecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: QuasarTextConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.config = config
        self.layer_type = config.layer_types[layer_idx]
        if self.layer_type == "linear_attention":
            self.linear_attn = QuasarGatedDeltaNet(config, layer_idx)
            ...
            if use_gla:
                print(f"  [L{layer_idx}] Initializing Quasar Gated-Linear Attention...")
                self.self_attn = QuasarGatedLinearAttention(config, layer_idx)
            else:
                self.self_attn = QuasarAttention(config, layer_idx)
        self.mlp = QuasarMLP(config, config.intermediate_size)
...
class QuasarPreTrainedModel(PreTrainedModel):
    def _set_state_dict_hook(self, state_dict, prefix, local_metadata, strict, missing_keys, unexpected_keys, error_msgs):
        ...
        # If we are loading QuasarForCausalLM, prefix is "" (root).
    ...
    _no_split_modules = ["QuasarDecoderLayer", "QuasarVisionBlock"]
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, QuasarGatedDeltaNet):
            init.ones_(module.dt_bias)
            init.copy_(module.A_log, torch.empty_like(module.A_log).uniform_(0, 16).log_())
        # We initialize with 0s to be 1 centered as the RMSNorm here does (1 + weight)
        elif isinstance(module, QuasarRMSNorm):
            init.zeros_(module.weight)
        elif isinstance(module, QuasarVisionRotaryEmbedding):
            inv_freq = 1.0 / (module.theta ** (torch.arange(0, module.dim, 2, dtype=torch.float) / module.dim))
            init.copy_(module.inv_freq, inv_freq)


class QuasarVisionMLP(nn.Module):
...
class QuasarVisionPatchEmbed(nn.Module):
...
class QuasarVisionPatchMerger(nn.Module):
    def __init__(self, config: QuasarVisionConfig, use_postshuffle_norm=False) -> None:
        super().__init__()
...
class QuasarVisionAttention(nn.Module):
    def __init__(self, config: QuasarVisionConfig) -> None:
        super().__init__()
...
class QuasarVisionBlock(GradientCheckpointingLayer):
    def __init__(self, config, attn_implementation: str = "sdpa") -> None:
        super().__init__()
        self.norm1 = nn.LayerNorm(config.hidden_size, eps=1e-6)
        self.norm2 = nn.LayerNorm(config.hidden_size, eps=1e-6)
        self.attn = QuasarVisionAttention(config=config)
        self.mlp = QuasarVisionMLP(config=config)
...
class QuasarVisionModel(QuasarPreTrainedModel):
    config: QuasarVisionConfig
    input_modalities = ("image", "video")
    _no_split_modules = ["QuasarVisionBlock"]
    _can_record_outputs = {
        "hidden_states": QuasarVisionBlock,
        "attentions": QuasarVisionAttention,
    }
    ...
    def __init__(self, config, *inputs, **kwargs) -> None:
        ...
        self.patch_embed = QuasarVisionPatchEmbed(
            config=config,
        )
        ...
        self.rotary_pos_emb = QuasarVisionRotaryEmbedding(head_dim // 2)

        self.blocks = nn.ModuleList([QuasarVisionBlock(config) for _ in range(config.depth)])
        self.merger = QuasarVisionPatchMerger(
            config=config,
...
class QuasarModelOutputWithPast(ModelOutput):
    r"""
...
class QuasarTextModel(QuasarPreTrainedModel):
    def __init__(self, config: QuasarTextConfig):
        ...
        self.rotary_emb = QuasarTextRotaryEmbedding(config=config)
        self.gradient_checkpointing = False
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if use_cache and past_key_values is None:
            past_key_values = QuasarDynamicCache(config=self.config)
...
class QuasarModel(QuasarPreTrainedModel):
    ...
    def __init__(self, config):
        super().__init__(config)
        self.visual = QuasarVisionModel._from_config(config.vision_config)
        self.language_model = QuasarTextModel._from_config(config.text_config)
    ...
    def forward(
        ...
        cache_position: torch.LongTensor | None = None,
        **kwargs: Unpack[TransformersKwargs],
    ) -> tuple | QuasarModelOutputWithPast:
        r"""
        image_grid_thw (`torch.LongTensor` of shape `(num_images, 3)`, *optional*):
            The temporal, height and width of feature shape of each image in LLM.
        video_grid_thw (`torch.LongTensor` of shape `(num_videos, 3)`, *optional*):
...
@auto_docstring
class QuasarForCausalLM(QuasarPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        if hasattr(config, "text_config"):
            config = config.text_config
        super().__init__(config)
        self.model = QuasarTextModel(config)
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        ```python
        >>> from transformers import AutoTokenizer, QuasarForCausalLM

        >>> model = QuasarForCausalLM.from_pretrained("Qwen/Quasar-8B")
        >>> tokenizer = AutoTokenizer.from_pretrained("Qwen/Quasar-8B")
...
@auto_docstring(
    custom_intro="""
    Base class for Quasar causal language model (or autoregressive) outputs.
    """
)
class QuasarCausalLMOutputWithPast(ModelOutput):
    r"""
...
class QuasarForConditionalGeneration(QuasarPreTrainedModel, GenerationMixin):
    config_class = QuasarConfig
    config_class = QuasarConfig
    config_class = QuasarConfig
    config_class = QuasarConfig
    ...
    def forward(
        ...
        logits_to_keep: int | torch.Tensor = 0,
        **kwargs: Unpack[TransformersKwargs],
    ) -> tuple | QuasarCausalLMOutputWithPast:
        r"""
        labels (`torch.LongTensor` of shape `(batch_size, sequence_length)`, *optional*):
            Labels for computing the masked language modeling loss. Indices should either be in `[0, ...,
            config.vocab_size]` or -100 (see `input_ids` docstring). Tokens with indices set to `-100` are ignored
        ...
        ```python
        >>> from transformers import AutoProcessor, QuasarForConditionalGeneration

        >>> model = QuasarForConditionalGeneration.from_pretrained("Qwen/Qwen3-VL-8B-Instruct")
    ...
    def _expand_inputs_for_generation(
        ...
        **model_kwargs,
        ...
        # Overwritten -- Quasar use timestamps and remove second_per_grid_ts
        ...
        def _expand_dict_for_generation_visual(dict_to_expand):
            ...
            # but Quasar append vision_start to each frame of each video, so we need to recover the real video_nums according to video_grid_thw
...
__all__ = [
    ...
    "QuasarModel",
    "QuasarForCausalLM",
    "QuasarForConditionalGeneration",
